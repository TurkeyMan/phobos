...
*/
alias Promote(T) = ModifyTypePreservingSTC!(PromoteImpl, OriginalType!T) Promote;
...
private template PromoteImpl(T)
{
    static if (is(T == ubyte)) alias ushort PromoteImpl;
        ...
        static assert(false, "Type '" ~ T.stringof ~ "' is already the largest possible type");
    else
        static assert(false, "Type '" ~ T.stringof ~ "' can not be promoted");
}
...
T must be a primitive built-in type, otherwise a compile-time error occurs.
*/
alias Demote(T) = ModifyTypePreservingSTC!(DemoteImpl, OriginalType!T) Demote;
...
private template DemoteImpl(T)
{
    static if (is(T == ulong)) alias uint DemoteImpl;
        ...
        static assert(false, "Type '" ~ T.stringof ~ "' is already the smallest possible type");
    else
        static assert(false, "Type '" ~ T.stringof ~ "' can not be demoted");
}
